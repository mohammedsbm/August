 ...
 */
public class AddUserWindow implements ActionListener {
    ...
    public AddUserWindow() {
        ...
        JFrame addUserFrame = new JFrame("Add User"); //create frame
        addUserFrame.setSize(800, 400); //set size of frame fit in the middle
        addUserFrame.setResizable(false); //non resizeable
        addUserFrame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
        //create popup window are you sure
        WindowListener l = new WindowAdapter() {
            public void windowClosing(WindowEvent e) {
                int confirm = JOptionPane.showOptionDialog(addUserFrame, //give user option to terminate
        ...
        Dimension di = addUserFrame.getToolkit().getScreenSize();
        //get window to be in the middle of the screen
        addUserFrame.setLocation(di.width/2 - addUserFrame.getWidth()/2, di.height/2 - addUserFrame.getHeight()/2); //get height/width of screen and divide by height/width of window

        addUserFrame.addWindowListener(l); //listening for the option to exit the frame

        addUserFrame.setJMenuBar(createMenu()); //set the menu bar

        content = (JPanel) addUserFrame.getContentPane(); //add this panel into the frame
