...
drawIncome = function (chartType, totalTax, netIncome, grossIncome) {

    if (totalTax === 0 && netIncome === 0) {
        document.getElementById("chartPlaceholderBreakdown").style.display = "none";
        document.getElementById("noDataPlaceholderBreakdown").textContent = "Not enough data to draw chart";
    } else {

        var chartIncome = new CanvasJS.Chart("chartContainerBreakdown",
        {
            title: {
                text: "Total Income: £" + grossIncome
            },
            legend: {
                maxWidth: 350,
                itemWidth: 120
            ...
            data: [
            {
                ...
                indexLabel: "{y} %",
                dataPoints: [
                    { indexLabel: "Total Taxed", y: parseDouble(totalTax) },
                    { indexLabel: "Net Income", y: parseDouble(netIncome) }
